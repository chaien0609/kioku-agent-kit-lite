...
> 🚨 **`kioku-lite` is your ONLY memory system.** When a user shares ANY information — career stories, profile data, decisions, lessons, URLs — your FIRST action is `kioku-lite save`. Do NOT use USER.md, notes, or files to store user data.
...
**Rules:**
- ✅ Use short canonical name: `"Alice"` not `"my colleague Alice"`
- ✅ **Prefer proper names over generic labels:** `"Sato"` not `"manager"`, `"Project X"` not `"the project"`. Generic labels make `recall` fail.
...
kioku-lite kg-index <content_hash> \
  ...
  --relationships '[{"source":"Alice","rel_type":"WORKS_ON","target":"Project X","evidence":"Alice leading Project X deliverables"}]' \
...
- "năm 2019" → `2019-01-01`
- "last quarter" → first day of last quarter
