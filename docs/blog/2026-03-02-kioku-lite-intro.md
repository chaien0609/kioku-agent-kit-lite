# Kioku Lite: Khi AI agent thực sự trở thành người bạn đồng hành cùng trí nhớ nhân quả
...
Mục tiêu là tool này phải **dùng được cho đa dạng agent nhất có thể**. Đó là lý do mình chọn kiến trúc giao tiếp qua **CLI** và **SKILLS file** (cụ thể là định dạng `AGENTS.md` & `SKILL.md` đang rất phổ biến hiện nay của dòng CLI Agent). Chỉ bằng vài dòng lệnh, agent có thể học cách gọi CLI để đọc/ghi ký ức.
...
Điều này nghĩa là **người dùng có thể tự custom các persona khác nhau** (thông qua cách Agent trích xuất). Ví dụ:
- Lắp persona "Companion", Agent sẽ extract các node `EMOTION`, `LIFE_EVENT` và nối chúng bằng `TRIGGERED_BY`.
- Lắp persona "Mentor", Agent sẽ extract `DECISION`, `LESSON` và quan hệ `LED_TO`.
...
Mình đã chuẩn bị sẵn hai văn bản hướng dẫn cài đặt mẫu cho 2 persona mà mình dùng nhiều nhất hàng ngày:
